...
RoutingTable::lookup(uint32_t ip) const
{
  ...
  RoutingTableEntry max_prefix_entry;
  bool found = false;
  //Find longest prefix in 
  for (auto it = m_entries.begin(); it != m_entries.end(); it++){
    //Compare prefixes, if prefixes match see if is the longest prefix
    if((ip & it->mask) == (it->gw & it->mask)) {
      if(max_prefix_len <= it->mask) {
        found = true;
        max_prefix_len = it->mask;
        max_prefix_entry = *it;
      }
    }
  }
  if(found)
  {
    // cerr << "target ip: " << ipToString(ip) << "\tmatched ip: " << ipToString(max_prefix_entry.gw) << endl;
    return max_prefix_entry;
  }
